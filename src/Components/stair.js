...
const Stair = (props) => {
  ...
  let totalRun = props.units
    ...
    : parseFloat(props.totalRunmm) >= 558.8 &&
      parseFloat(props.totalRunmm) <= 4875.2125
    ? parseFloat(props.totalRunmm) / 25.4
    : 3658 / 25.4;

  let totalRise = props.units
    ...
    : parseFloat(props.totalRisemm) >= 279.4 &&
      parseFloat(props.totalRisemm) <= 3351.2125
    ? parseFloat(props.totalRisemm) / 25.4
    : 2743 / 25.4;

  let idealRun = props.units
    ? parseFloat(props.idealRunin) + parseFloat(props.idealRunfr)
    : parseFloat(props.idealRunmm) >= 254 &&
      parseFloat(props.idealRunmm) <= 288.9375
    ? parseFloat(props.idealRunmm) / 25.4
    : 279.4 / 25.4;

  let idealRise = props.units
    ? parseFloat(props.idealRisein) + parseFloat(props.idealRisefr)
    : parseFloat(props.idealRisemm) >= 101.6 &&
      parseFloat(props.idealRisemm) <= 196.85
    ? parseFloat(props.idealRisemm) / 25.4
    : 177.8 / 25.4;

  let preTreadThickness = props.units
    ? props.details
  ...
  let preRiserThickness = props.units
    ? props.details
  ...
  let stringerA = stringerTotal / Math.sin(Math.atan(idealRun / idealRise));

  let stringerB = stringerTotal / Math.sin(Math.atan(idealRise / idealRun));
  ...
  if (props.details) {
    ...
    stringerA = stringerTotal / Math.sin(Math.atan(idealRun / idealRise));
    stringerB = stringerTotal / Math.sin(Math.atan(idealRise / idealRun));
  }

  let coordinates = [];

  let floorThickness = props.units
  ...
  if (props.units === false) {
    if (
      parseFloat(props.floormm) >= 152.4 &&
      parseFloat(props.floormm) <= 608.0125
    ) {
  ...
  let preStairAngle =
    props.stairAngle >= 25 && props.stairAngle <= 45 ? props.stairAngle : 32.5;
  ...
  let windowWidth = lgMin.matches ? window.innerWidth * 0.9 : window.innerWidth;
  let windowHeight = lgMin.matches
    ? window.innerHeight
  ...
  let move = lgMin.matches ? [moveCenter - 60, 150] : [moveCenter - 30, 100];

  //Treads, risers, and nosing dimensions
  let treadThickness = ratio * treadTotal;
  let riserThickness = ratio * riserTotal;
  let nosing = ratio * nosingTotal;
  ...
  let dRiseInches = props.topStair
    ? Math.floor(
  ...
  let dRiseFraction = props.topStair
    ? Math.round(
        ((coordinates[coordinates.length - 11] - coordinates[1]) / ratio -
          dRiseFeet * 12 -
          dRiseInches) *
  ...
  let dRiseMM = props.topStair
    ? Math.abs(
        Math.round(
          ((coordinates[coordinates.length - 11] - coordinates[1]) * 25.4) /
            ratio
        )
      )
    : Math.abs(
  ...
  let dRunFraction = props.topStair
    ? Math.round(
        ((coordinates[0] - coordinates[coordinates.length - 12]) / ratio -
          dRunFeet * 12 -
          dRunInches) *
  ...
  let dRunMM = props.topStair
    ? Math.abs(
        Math.round(
          ((coordinates[0] - coordinates[coordinates.length - 12]) * 25.4) /
            ratio
        )
      )
    : Math.abs(
        Math.round(
          ((coordinates[0] - coordinates[coordinates.length - 8]) * 25.4) /
            ratio
        )
      );

  let dRunValue = props.units
  ...
  let dStringerValue = props.units
  ...
  let dNosing = props.topStair
    ? [coordinates[2], 0, coordinates[2] - nosing, 0]
    : [
        coordinates[4],
        idealRise * ratio,
        coordinates[4] - nosing,
        idealRise * ratio,
      ];
  ...
  let dNosingText = [
    (dNosing[0] + dNosing[2]) / 2,
    dNosing[1] - textSize * 2.5,
  ];
  let dNosingValue = props.units
  ...
  let dHeadroomStepY = props.topStair
    ? coordinates[coordinates.length - 9]
    : coordinates[coordinates.length - 5];
  ...
  let dHeadroomDashed = props.topStair
    ? [
        dHeadroomStepX,
        coordinates[coordinates.length - 9],
        coordinates[coordinates.length - 10],
  ...
  let dHeadroomValue = (
    (parseFloat(dHeadroomStepY) - parseFloat(dHeadroom[1])) /
    ratio
  ).toFixed(2);
  ...
  let dHeadroomValueText = props.units
  ...
  let dAngleStart = props.topStair
    ? [
        coordinates[coordinates.length - 12],
  ...
  let dAngleText = props.topStair
    ? [coordinates[coordinates.length - 10] + (idealRun / 2) * ratio, dAngle[1]]
    : [coordinates[coordinates.length - 6] + (idealRun / 2) * ratio, dAngle[1]];
  ...
  let idealRunParsedIn = props.topStair
    ? Math.floor((coordinates[12] - coordinates[14]) / ratio)
    : Math.floor((coordinates[6] - coordinates[10]) / ratio);

  let idealRunParsedFr = props.topStair
    ? Math.round(
        ((coordinates[12] - coordinates[14]) / ratio - idealRunParsedIn) *
          0.0625
      )
    : Math.round(
        ((coordinates[6] - coordinates[10]) / ratio - idealRunParsedIn) * 0.0625
      );

  let idealRunParsedMM = props.topStair
    ? Math.round(((coordinates[12] - coordinates[14]) / ratio) * 25.4)
    : Math.round(((coordinates[6] - coordinates[10]) / ratio) * 25.4);

  let idealRiseParsedIn = props.topStair
    ? Math.floor((coordinates[11] - coordinates[7]) / ratio)
    : Math.floor((coordinates[15] - coordinates[11]) / ratio);

  let idealRiseParsedFr = props.topStair
    ? Math.round(
        ((coordinates[11] - coordinates[7]) / ratio - idealRunParsedIn) * 0.0625
      )
    : Math.round(
        ((coordinates[15] - coordinates[11]) / ratio - idealRunParsedIn) *
          0.0625
      );

  let idealRiseParsedMM = props.topStair
    ? Math.round(((coordinates[11] - coordinates[7]) / ratio) * 25.4)
    : Math.round(((coordinates[15] - coordinates[11]) / ratio) * 25.4);

  let riseCountValue = props.units
    ? runCount +
      " rises at " +
      idealRiseParsedIn +
      '" ' +
  ...
  //Error message texts
  let errors = [];
  let ePosition = lgMin.matches ? [80, 80] : [8, 40];
